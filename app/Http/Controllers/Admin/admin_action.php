...
class admin_action extends Controller
{
    ...
    public function modify_profile_buttons(Request $request, $id)
    {
        ...
        switch ($action) {
            ...
            case 'account-status':
                // Handle account status action
                $user->account_status = $user->account_status === 'active' ? 'inactive' : 'active';
                $user->save();
                return redirect()->back()->with('success', 'User account status successfully updated.');
                break;
    ...
    public function wallet_express_percentage(Request $request){
        $validatedData = request()->validate([
            'express_percentage' => 'required|numeric|min:0|max:100', // Ensures it's a required number between 0 and 100
        ]);
        $adminWallet = Adminwallet::first(); // Retrieve the first row from the Adminwallet table (adjust this as needed)

        $adminWallet->express_percentage = request()->input('express_percentage');
        $adminWallet->save();

        return redirect()
            ->back()
            ->with('success', 'Express percentage updated successfully.');
    }
}
