...
class Userwithdrawal extends Controller
{
    ...
    public function use_express($id, Request $request)
    {
        $withdrawal = Userwithdraw::find($id);
        
        if (!$withdrawal) {
            return redirect()->back()->withErrors(['message' => 'Withdrawal request not found.']);
        }else{
            // Fetch the express withdrawal percentage from the admin table
            $adminWallet = Adminwallet::first(); // Retrieve the first row from the Adminwallet table (adjust this as needed)
            $percentage = $adminWallet->express_percentage;
            $express_amount = ($withdrawal->amount * $percentage) / 100;

            

            // Send data to the view
            return view('express_payment', compact('withdrawal', 'percentage'));
