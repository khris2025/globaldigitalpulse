                                 ...
                                 <div class="col-xl-4 col-sm-6">
                                    <div class="card mb-4 text-center shadow-sm">
                                       <div class="card-body">
                                          <div class="p-3">
                                             {{-- Crypto Logo --}}
                                             <img src="{{ asset('storage/mining_img/' . $plan->Coin_img) }}" alt="{{ $plan->name }} Logo" class="mb-3" style="width: 100px; height: 100px; object-fit: contain;">

                                             <h5 class="font-size-18 fw-semibold">{{ $plan->name }}</h5>
                                             ...
                                             <div class="text-muted mb-4">
                                                <p class="mb-2 font-size-15">
                                                   <i class="mdi mdi-clock-outline text-primary me-2"></i>Duration: {{ $plan->duration }} Days
                                                </p>
                                                <p class="mb-0 font-size-15">
                                                   <i class="mdi mdi-currency-usd text-success me-2"></i>ROI: {{ $plan->roi }}%
                                                </p>
                                             </div>

                                             {{-- Subscription Form --}}
                                             <form action="{{ route('user.subscribe.miningplan', $plan->id) }}" method="POST" class="mt-3">
                                                @csrf
                                                <div class="form-group mb-3">
                                                   <input type="number" name="amount" class="form-control text-center" placeholder="Enter amount to invest" required min="{{ $plan->min_amount }}" max="{{ $plan->max_amount }}">
                                                </div>
                           ...
                           <div class="table-responsive px-3" data-simplebar >
                              <table id="datatable" class="table nowrap w-100">
                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                       <td style="font-size: 16px;" class="font-w400">{{ $subscribedPlan->created_at->format('F j, Y')  }}</td>
                                       <td style="font-size: 16px;" class="font-w400">{{ $subscribedPlan->transid }}</td>
                                       <td style="font-size: 16px;" class="font-w400">{{ $subscribedPlan->plan }}</td>
                                       <td style="font-size: 16px;" class="font-w400">${{ number_format($subscribedPlan->amount) }}</td>
                                       <td style="font-size: 16px;" class="font-w400">{{ $subscribedPlan->Withdrawaldate->format('F j, Y') }}</td>
                                       <td style="font-size: 16px;" class="font-w400">${{ number_format($subscribedPlan->profit) }}</td>
                                       <td>
                                          <button type="button " class="btn btn-rounded btn-sm btn-outline-warning">
                                          <i class="bx bx-hourglass bx-spin font-size-16 align-middle me-2"></i>{{ $subscribedPlan->status }}
                                          </button>
                                       </td>
                                    </tr>
                                    @endforeach
                  ...
                  <div class="card-body px-0">
                     <div class="px-3" data-simplebar style="max-height: 352px;">
                        <ul class="list-unstyled activity-wid mb-0">
                           @foreach ($completedmining as $completedmining)
                              ...
                              <div class="timeline-list-item">
                                 <div class="d-flex">
                                    <div class="flex-grow-1 overflow-hidden me-4">
                                       <h5 class="font-size-14 mb-1">Transaction ID - {{ $completedmining->transid }}</h5>
                                    </div>
                                    <div class="flex-shrink-0 text-end me-3">
                                       <h6 class="mb-1">Investment: <span class="text-success"> ${{ number_format($completedmining->amount) }} </span> </h6>
                                       <div class="font-size-13">profit: ${{ number_format($completedmining->profit) }} </div>
                                    </div>
                                 </div>
                              </div>
                           </li>
